...
use gtfs::{route_section_id, write_gtfs};
...
fn sample<T: Clone>(xs: Vec<T>, size: usize) -> Vec<T> {
    ...
    println!(
        "{}: {}..{}",
        Green.bold().paint("Sample window"),
        lower,
        upper
    );
        ...
        for line in &mut lines {
            ...
            scope.execute(move || {
                ...
                for route_section in &mut line.route_sections {
                    ...
                    println!(
                        "\tRoute Section: {} ...",
                        White.bold().paint(route_section.name.clone())
                    );
                    route_section.timetable = client.get_timetable(
                        &line.id,
                        &route_section.originator,
                        &route_section.destination,
                    )
                }
            });
        }
...
fn transform_gtfs(lines: Vec<Line>) {
    ...
    for line in &lines {
        ...
        for route_section in &line.route_sections {
            ...
            println!(
                "     {}, Has Timetable: {}, Duplicate: {}",
                id,
                has_timetable,
                route_section_ids.contains(&id)
            );
    ...
    println!(
        "Duplicate Lines: {}, Duplicate Route Sections: {}",
        line_count - line_ids.len(),
        route_section_count - route_section_ids.len()
    );
